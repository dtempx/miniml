...
**What is this for?** MiniML basically enables the AI scenarios of *text-to-SQL* or *chat-with-your-data*. You define your data once in a simple YAML model, and MiniML gives an AI (or your app) a safe, structured vocabulary of dimensions and measures to work with. Instead of letting an LLM write raw SQL against your raw tables—which can be error-prone and risky—the AI picks from your well-defined model, and MiniML generates correct, optimized SQL for you. The result: users can ask questions in plain English like *"What were total sales by store last month?"* and get accurate answers from your database.
