...
A minimal semantic data modeling language for generating SQL queries from YAML model definitions.

MiniML produces SQL queries from dimensions and measures defined in a simple YAML model syntax. It's designed to be a super simple library, not a heavy environment, with support for BigQuery and Snowflake dialects.
...
```bash
npx miniml test.yaml --dimensions=date --measures=revenue
```
...
FROM my_table
WHERE created_at BETWEEN '2024-11-09' AND '2024-12-09'
GROUP BY ALL
```
